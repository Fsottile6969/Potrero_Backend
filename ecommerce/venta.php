<!DOCTYPE html>
<html lang="en">
	<head>
		<meta charset="UTF-8" />
		<meta http-equiv="X-UA-Compatible" content="IE=edge" />
		<meta name="viewport" content="width=device-width, initial-scale=1.0" />
		<link
			href="https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/css/bootstrap.min.css"
			rel="stylesheet"
			integrity="sha384-1BmE4kWBq78iYhFldvKuhfTAU6auU8tT94WrHftjDbrCEXSU1oBoqyl2QvZ6jIW3"
			crossorigin="anonymous"
		/>
		<title>Perfumeria online</title>
	</head>
	<body style="overflow-x: hidden">
		<div class="container-fluid">
			<div class="row d-flex justify-content-between">
				<div class="col-md-8 d-flex justify-content-end">
					<h1
						class="titlee text-danger fst-italic fw-bolder p-3 d-inline-flex"
					>
						Fragance - Donde tu aroma destaca
					</h1>
				</div>
			</div>
		</div>

		<nav class="navbar navbar-expand-lg navbar-light bg-danger bg-opacity-50">
			<div class="container-fluid w-auto">
				<button
					class="navbar-toggler"
					type="button"
					data-bs-toggle="collapse"
					data-bs-target="#navbarNavDropdown"
					aria-controls="navbarNavDropdown"
					aria-expanded="false"
					aria-label="Toggle navigation"
				>
					<span class="navbar-toggler-icon"></span>
				</button>
				<div
					class="collapse navbar-collapse text-center"
					id="navbarNavDropdown"
				>
					<ul class="navbar-nav">
						<li class="nav-item">
							<a class="nav-link text-capitalize fw-bolder" href="catalogo.php"
								>catalogo</a
							>
						</li>
						<li class="nav-item">
							<a class="nav-link text-capitalize fw-bolder" href="hombre.php"
								>hombre</a
							>
						</li>
						<li class="nav-item">
							<a class="nav-link text-capitalize fw-bolder" href="mujer.php"
								>mujer</a
							>
						</li>
						<li class="nav-item">
							<a href="listar.php" class="nav-link text-capitalize fw-bolder"
								>listado interno</a
							>
						</li>
						<li class="nav-item">
							<a href="crearArt.html" class="nav-link text-capitalize fw-bolder"
								>Agregar perfume</a
							>
						</li>
						<li class="nav-item">
							<a href="modificar.php" class="nav-link text-capitalize fw-bolder"
								>modificar</a
							>
						</li>
					</ul>
				</div>
			</div>
		</nav>

		<h3 class="pasos text-center text-danger m-5 pt-3">
			Crea tu tienda en tres simples pasos!!!
		</h3>
		<div class="row">
			<div class="col d-flex flex-row justify-content-evenly mt-5">
				<div class="preCard opacity-75 mx-3 d-flex">
					<div class="card bg-danger opacity-100" style="width: 18rem">
						<p
							class="text-card h1 text-uppercase text-center text-warning fw-bolder mt-4"
						>
							paso 1
						</p>
						<div class="card-body">
							<p class="card-text text-center text-white pt-0 p-4">
								Registrate para poder administrar tu catalogo.
							</p>
						</div>
					</div>
				</div>
				<div class="preCard opacity-75 mx-3 d-flex">
					<div class="card bg-danger opacity-100" style="width: 18rem">
						<p
							class="text-card h1 text-uppercase text-center text-warning fw-bolder mt-4"
						>
							paso 2
						</p>
						<div class="card-body">
							<p class="card-text text-center text-white pt-0 p-4">
								Agrega la cantidad de productos que desees.
							</p>
						</div>
					</div>
				</div>
				<div class="preCard opacity-75 mx-3 d-flex">
					<div class="card bg-danger opacity-100" style="width: 18rem">
						<p
							class="text-card h1 text-uppercase text-center text-warning fw-bolder mt-4"
						>
							paso 3
						</p>
						<div class="card-body">
							<p class="card-text text-center text-white pt-0 p-4">
								Ya tenes tu propia tienda online lista para vender.
							</p>
						</div>
					</div>
				</div>
			</div>
		</div>
		<script
			src="https://cdn.jsdelivr.net/npm/@popperjs/core@2.10.2/dist/umd/popper.min.js"
			integrity="sha384-7+zCNj/IqJ95wo16oMtfsKbZ9ccEh31eOz1HGyDuCQ6wgnyJNSYdrPa03rtR1zdB"
			crossorigin="anonymous"
		></script>
		<script
			src="https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/js/bootstrap.min.js"
			integrity="sha384-QJHtvGhmr9XOIpI6YVutG+2QOK9T+ZnN4kzFN1RtK3zEFEIsxhlmWl5/YESvpZ13"
			crossorigin="anonymous"
		></script>
	</body>
</html>
